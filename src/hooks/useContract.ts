...
export const useBuyHandwithHodlx = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getBuyHandwithHodlxContract(library.getSigner()), [library])
}

export const useBuyHandwithHodl = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getBuyHandwithHodlContract(library.getSigner()), [library])
}
