...
export function toV2LiquidityToken([tokenA, tokenB]: [Token, Token]): Token {
  if (
    tokenA.address == '0x3e4Aa1F08E1fd94d6583c7C776cEB4BA68784aa7' &&
    tokenB.address == '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c'
  ) {
    return new Token(tokenA.chainId, '0x0bd7F4AEBed7b748E4743A8C544B7C5450dD7EBa', 18, 'Cake-LP', 'Pancake LPs')
  }
  return new Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'Hodlx-LP', 'Hodlx LPs')
}
...
export function useUserAgreedTermsAndConditions(): [boolean, (agreedTermsAndConditions: boolean) => void] {
  const dispatch = useDispatch<AppDispatch>()
  const userAgreedTermsAndConditions = useSelector<AppState, AppState['user']['agreedTermsAndConditions']>((state) => {
    return state.user.agreedTermsAndConditions
  })

  const setUserAgreedTermsAndConditions = useCallback(
    (agreedTermsAndConditions: boolean) => {
      dispatch(setAgreedTermsAndConditions(agreedTermsAndConditions))
    },
    [dispatch],
  )

  return [userAgreedTermsAndConditions, setUserAgreedTermsAndConditions]
}
