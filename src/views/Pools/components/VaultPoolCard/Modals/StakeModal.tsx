...
const StakeModal: React.FC<StakeModalProps> = ({
  ...
  const handleConfirmClick = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      if (isRemovingStake) {
        return onUnstake(stakeAmount, stakingToken.decimals, pool.isNft)
      }
      return onStake(stakeAmount, stakingToken.decimals, pool.isNft)
    })
    if (receipt?.status) {
      if (isRemovingStake) {
        toastSuccess(
        ...
        toastSuccess(
          `${t('Staked')}!`,
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('Your %symbol% have been staked in the pool!', {
  ...
  return (
    ...
    >
      ...
      <Text ml="auto" color="textSubtle" fontSize="12px" mb="8px">
        {t('Balance: %balance%', {
          balance: Number(getFullDisplayBalance(getCalculatedStakingLimit(), stakingToken.decimals, 0)).toLocaleString(
            undefined,
      ...
      {isRemovingStake && !pool.isNft ? (
        <Flex maxWidth="346px" mt="24px">
          <Text textAlign="center">
            {pool.stakingToken.symbol === 'HODLX'
              ? t(
                'When unstaking you’ll also receive your rewards, and when you unstake in full you’ll also receive back your NFTs! Note – there is a 2.5% tax on the %symbol% you’ve staked and earned when withdrawing in the first 30 days.',
                { symbol: pool.stakingToken.symbol },
              )
              : t(
                'There is no tax when unstaking.',
              )}
          </Text>
        </Flex>
      ) : (
        <Flex maxWidth="346px" mt="24px">
          <Text textAlign="center">
            {pool.stakingToken.symbol === 'HODLX'
              ? t('The required amount of NFTs will be staked along with your %symbol%.', {
                symbol: pool.stakingToken.symbol,
              })
              : t(
                'This pool currently has no end date, however when staking $HODL, the tokens are locked for the first 90 days. Note - This lock period will be reset to a new period of 90 days if you add more $HODL at any time.',
              )}
          </Text>
        </Flex>
      )}
        ...
        <>
          <StyledLink external href={getTokenLink}>
            <Button width="100%" mt="8px" variant="secondary">
              {t('Buy %symbol%', { symbol: stakingToken.symbol })}
            </Button>
          </StyledLink>
          <StyledLink external href={mainnetTokens.hh.projectLink}>
            <Button width="100%" mt="8px" variant="secondary">
              {t('Buy NFTs', { symbol: stakingToken.symbol })}
            </Button>
          </StyledLink>
        </>
      )}
