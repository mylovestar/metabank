...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const discountSave = pool3.pool.stakingTokenPrice ? new BigNumber(discountRatePercent).div(new BigNumber(100)).times(new BigNumber(pool3.pool.stakingTokenPrice)) : new BigNumber(0)
  ...
  return (
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0 && sousId !== 15}>
        <PoolCardHeaderTitle
          title={t('Buy NFTs with HODL')}
          subTitle={t('Get a discount when buying with HODL')}
        />
