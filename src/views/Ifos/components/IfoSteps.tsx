  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Stake HODLX in IFO pool')}
      </Heading>
      <Box>
        <Text color="textSubtle" small>
          {t(
            'The maximum amount of HODLX user can commit to the Public Sale, is equal to the average HODLX balance in the IFHODLXDLX pool prior to the IFO. Stake moHODLXODLX to increase the maxiHODLXHODLX you can commit to the sale. Missed this IFO? You can keep staking in theHODLX HODLX Pool to join the next IFO sale.',
          )}
        </Text>
      ...
      {hasProfile && (
        <SmallStakePoolCard borderRadius="default" p="16px">
          <FlexGap justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="16px">
            ...
            {isVaultApproved ? (
              <Button onClick={onPresentStake}>{t('Stake')} HODLX</Button>
            ) : (
...
const Step2 = ({ hasProfile, isLive, isCommitted }: { hasProfile: boolean; isLive: boolean; isCommitted: boolean }) => {
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can “commit” your HODLX to buy the tokens being sold.')} <br />
      </Text>
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent HODLX tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )
