...
export default function Swap() {
  ...
  const [defaultSlippage, setDefaultSlippageTolerance] = useState<boolean>(false)
  const [allowedSlippage, setUserSlippageTolerance] = useUserSlippageTolerance()
  /*
  const [allowedSlippage] = (currencies.INPUT.symbol == "HODL" || currencies.OUTPUT.symbol == "HODL") ? 
                              [1200] : (currencies.INPUT.symbol == "HODLX" || currencies.OUTPUT.symbol == "HODLX") ?
                              [10] : useUserSlippageTolerance()
                              */

  if (!defaultSlippage) {
    setUserSlippageTolerance(10)
    setDefaultSlippageTolerance(true)
  }
  // Price data
  const {
  ...
  const [inputSymbol, setInputSymbol] = useState('BNB')
  const [outputSymbol, setOutputSymbol] = useState('MBC')
  ...
  // console.log("debug formatted", formattedAmounts, )
  ...
  const handleOutputSelect = useCallback(
    (outputCurrency) => {
      setOutputSymbol(outputCurrency.symbol)
  ...
  useEffect(() => {
    if (inputSymbol === 'HODL' || outputSymbol === 'HODL') {
      if (allowedSlippage < 1200) setUserSlippageTolerance(1200)
    } else if (inputSymbol === 'MBC' || outputSymbol === 'MBC') {
      setUserSlippageTolerance(10)
    }
                      ...
                      <AutoColumn justify="space-between">
                        <AutoRow justify={isExpertMode ? 'space-between' : 'center'} style={{ padding: '0 1rem' }}>
                          <SwitchIconButton
                            ...
                            onClick={() => {
                              setApprovalSubmitted(false) // reset 2 step UI for approvals
                              setOutputSymbol(currencies[Field.INPUT].symbol)
                              setInputSymbol(currencies[Field.OUTPUT].symbol)
                              onSwitchTokens()
                            }}
                          >
                            <ArrowDownIcon
